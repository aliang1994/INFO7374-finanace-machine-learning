...
def kf_train(Y):
    ...
    #residuals 
    res = Y - x_smooth[:, 0].reshape(-1)
    
    
    x_update.shape=(Y.shape[0],2,)
    x_smooth.shape=(Y.shape[0],2,)
    
    return x_smooth[:, 0], res


def kf_cv(y_bstr):
    y_bstr = transpose(array(y_bstr))
    #print("y_bstr", y_bstr)
    T = y_bstr.shape[0]
    ...
    x_update.shape=(y_bstr.shape[0],2,)
    x_smooth.shape=(y_bstr.shape[0],2,)
  
    y_pred = A @ x_smooth[T-1]
    #print("x_smooth[T-1]: ", x_smooth[T-1])
    #print("y_pred: ", y_pred)
